...
- Add a stub Cargo workspace: root `Cargo.toml` (`[workspace]` with no package of its own, `resolver = "3"`) and one member crate `crates/identus-ssi/` (`Cargo.toml` + placeholder `src/lib.rs`, `edition = "2024"`, `rust-version = "1.85.0"`) so the crane-based checks have a real cargo project to exercise and so future proposals have a landing zone.
...
- Toolchain: stable only (with `wasm32-unknown-unknown` target included). No nightly toolchain is provided; nightly is deferred to the proposal that first proves a nightly need.
