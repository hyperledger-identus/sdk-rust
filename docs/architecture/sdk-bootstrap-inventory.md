...
| `identus-oid4vci` | bounded Final Credential Offer, metadata, pre-authorized Token Request/Response, nonce, configuration-ID/JWT-proof Credential Request, and immediate Credential Response core semantics with static errors and zeroizing secret ownership | no HTTP execution, Authorization Details/Credential identifiers, token/proof/credential trust or freshness, error/deferred/encrypted responses, format/chain extensions, storage or product policy; direct internal cone is `identus-core` plus `identus-jose` | bounded child deliveries ending at issue #141 under #7 |
