# foundation-time Specification

## Purpose

`foundation-time` defines portable time values and narrow injected clock ports
for SDK capabilities that need civil or elapsed time without selecting an
operating system, async runtime, persistence format or concrete time library.
