...
The `sdk-rust` workspace single-sources external dependency versions in the root `Cargo.toml` `[workspace.dependencies]` block. Each consuming crate references an external dependency via `<dep>.workspace = true` rather than restating `version`. This keeps version bumps in one place, prevents drift across crates, and complements the `crate-ring-layout` dep-graph guard, which enforces the workspace-form rule in source.
