...
def require_tool_version(tool: str, output: str, expected: str) -> str:
    """Require an exact release tool version and return its normalized output."""
...
def require_preparation_toolchain(
    root: Path, descriptor: dict[str, Any], env: dict[str, str]
) -> tuple[str, str]:
    expected = descriptor["preparation_rust_version"]
    rust_version = require_tool_version(
        "rustc", run(["rustc", "--version"], cwd=root, env=env), expected
    )
    cargo_version = require_tool_version(
        "cargo", run(["cargo", "--version"], cwd=root, env=env), expected
    )
    return rust_version, cargo_version
...
def prepare(args: argparse.Namespace) -> Path:
    ...
    rust_version, cargo_version = require_preparation_toolchain(root, descriptor, env)
