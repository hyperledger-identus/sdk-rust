...
| `identus-jose` | bounded canonical JWS Compact parsing, exact signing input, external signer port, explicit algorithm-bound public JWKs, bounded verifier registry, fully specified Ed25519/ES256 software suites, opt-in legacy EdDSA, and explicit verified state | no claims, DID resolution/authorization, `kid` matching, trust, async/provider transport, embedded keys/certificates, JWS JSON, JWE or custody | issues #95 and #98 under #8; proof/DID profile #99 and fuzz #100 remain follow-ups |
