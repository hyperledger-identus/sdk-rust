...
| `identus-jose` | bounded canonical JWS Compact parsing, closed `alg`/`typ`/`kid` protected headers, exact signing-input preservation and staged external-signature attachment | no signature verification, positive algorithm policy, JWT claims, DID resolution, proof profiles, embedded keys/certificates, JWS JSON, JWE or custody | issue #95 under #8; builder/verifier/profile and fuzz slices remain follow-ups |
