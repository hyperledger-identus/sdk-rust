...
| `siros-dcql` | 0.3.0 | 1.82 | 12 resolved packages; one prospective incremental root name | `spike` / `conditional-adopt` | Cohesive pure-Rust OID4VP Final DCQL selection engine; the [completed evidence](../siros-dcql-spike/results.md) proves matching, isolation, measured payoff and mismatches, while unbounded/tolerant public models still require an Identus facade under ADR 0156. |
...
| 11 | [#162 — Askar](https://github.com/hyperledger-identus/sdk-rust/issues/162) | `not-adopt`; reference fixture only per ADR 0103 |
