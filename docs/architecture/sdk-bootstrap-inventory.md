...
| `identus-jose` | bounded canonical JWS Compact parsing, exclusive `kid`/public-`jwk`/`x5c` headers, exact signing input, external signer port, explicit algorithm-bound public JWKs, bounded verifier registry, Ed25519/ES256 suites, opt-in legacy EdDSA, explicit verified state, staged OpenID4VCI Final proof-JWT holder construction, and issuer verification through inline JWK, authenticated DID URL or injected X.509/replay policy | no certificate parsing/path validation, key attestation/trust chain, trust anchors, non-DID `kid`, multibase conversion, generic async/provider transport, JWS JSON, JWE or custody | issues #95, #98 and #99 under #8; optional proof extensions #104 and fuzz #100 remain follow-ups |
