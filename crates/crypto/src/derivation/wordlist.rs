//! BIP39 English wordlist (2048 words), embedded as a const slice so the
//! crate source is self-contained (no `include_str!` of a `".txt"` asset that
//! would be stripped by the workspace source filter).

/// The BIP39 English wordlist (2048 words).
pub const ENGLISH_WORDLIST: &[&str] = &[
