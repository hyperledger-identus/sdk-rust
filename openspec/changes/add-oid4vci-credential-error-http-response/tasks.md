...
- [x] 1.1 Create issue #147 before implementation and record exact base,
  normative hash, consumer revisions, public/wire/error contract, non-scope,
  bounds, targets, isolation, and rollback.
- [x] 1.2 Define and semantically review the additive HTTP capability and the
  complete IDR-023 replacement with zero unresolved blockers.

## 2. Implementation

- [x] 2.1 Add positive composed Credential Error HTTP limits and exact
  status/media validation over the existing bounded body parser.
- [x] 2.2 Reject exact generic `invalid_request`, preserve other extensions,
  and add fieldless redaction-safe bridge errors.
...
- [x] 3.1 Add positive, negative, ordering, boundary, extension, body-error,
  redaction, bridged-error, and consumer-shaped tests in both feature modes.
...
- [ ] 3.4 Prepare the signed/DCO issue-linked PR delivery packet for `develop`,
