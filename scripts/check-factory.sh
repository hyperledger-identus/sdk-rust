...
if [[ -x "$factory_root/scripts/check-oid4vci-conformance.py" && -f "$factory_root/docs/conformance/oid4vci-final-wallet-core.csv" ]]; then
  if ! "$factory_root/scripts/check-oid4vci-conformance.py" "$factory_root"; then
    report_failure "OID4VCI Final conformance-matrix validation failed"
  fi
fi
...
if [[ -x "$factory_root/scripts/check-release-candidates.py" ]]; then
  if ! "$factory_root/scripts/check-release-candidates.py" "$factory_root"; then
    report_failure "independent release-candidate validation failed"
  fi
fi
