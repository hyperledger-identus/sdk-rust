...
| `identus-credentials` | bounded format identifier, opaque artifacts/envelope, metadata/schema descriptors and canonical staged verification evidence | no claim values, serde/wire codecs, concrete formats/protocols, trust decisions, verifier/schema execution, holder/status bindings, display/localization or storage | issues #71, #73 and #75; later credential/verification slices remain experimental |
