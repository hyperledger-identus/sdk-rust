...
| `identus-oid4vci` | bounded Credential Offer invocation parsing, core issuer/configuration semantics, typed Final Authorization Code and Pre-Authorized Code grant shapes, Transaction Code requirements, lossless extensions, static errors and erased owned content | no fetching, metadata matching, grant selection, replay/freshness enforcement, issuer trust, SSRF policy, issuance state machine, authorization, token or credential endpoint messages | issues #111, #113 and #115 under #7 |
