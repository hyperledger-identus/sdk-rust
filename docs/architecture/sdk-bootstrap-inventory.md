...
| `identus-oid4vci` | bounded Final Credential Offer, metadata, pre-authorized Token Request/Response, nonce and configuration-ID/JWT-proof Credential Request semantics with static errors and zeroizing secret ownership | no HTTP execution, Authorization Details/Credential identifiers, token/proof trust or freshness, encryption, format/chain extensions, Credential Response processing or product policy; direct internal cone is `identus-core` plus `identus-jose` | bounded child deliveries ending at issue #139 under #7 |
