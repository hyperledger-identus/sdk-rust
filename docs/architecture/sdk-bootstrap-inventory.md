...
| `identus-jose` | bounded canonical JWS Compact parsing, exclusive `kid`/public-`jwk`/`x5c` headers, exact signing input, external signer port, explicit algorithm-bound public JWKs, bounded verifier registry, Ed25519/ES256 suites, opt-in legacy EdDSA, explicit verified state, and staged OpenID4VCI Final proof-JWT holder construction | no issuer proof verification, DID resolution/authorization, `kid` matching, certificate path validation, key attestation/trust chain, trust, async/provider transport, JWS JSON, JWE or custody | issues #95, #98 and holder half of #99 under #8; verifier half of #99 and fuzz #100 remain follow-ups |
