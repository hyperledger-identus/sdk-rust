...
issue reference from completed child #115 to active child #117. Issue #117
SHALL deliver only bounded unsigned Credential Issuer Metadata core semantics
and exact agreement with the existing grant-validated offer.

The slice SHALL NOT claim the full OID4VCI engine, metadata retrieval, signed
metadata, Authorization Server discovery, grant selection, token/credential
messages, replay safety, consumer adoption, publication, or release.

#### Scenario: ledger points at the active bounded child

- **WHEN** issue #117 is implemented
- **THEN** `IDR-023` references #117 with `delivery_status=in_progress`
- **AND** #7 and #20 remain the open component/program parents

#### Scenario: partial metadata delivery is not engine completion

- **WHEN** unsigned metadata and offer agreement gates pass
