...
one string `alg` member, at most one string `typ`, at most one key reference
selected from string `kid`, public object `jwk`, or array `x5c`, at most one
string `key_attestation`, and at most one array `trust_chain`. Unknown or
...
bound. `key_attestation` SHALL be a non-empty visible-ASCII three-segment
compact token fitting that bound. `trust_chain` SHALL contain 1 through 8 such
tokens. Encoding SHALL stop once the configured decoded protected-header byte
...
supported or compatible with a key. It SHALL NOT validate certificate paths,
nested JWTs, federation chains or trust. It SHALL preserve attestation and
trust-chain values only as bounded untrusted evidence for a focused profile.
It SHALL NOT accept `crit`, `b64` or any other unknown semantics.

#### Scenario: consumer-shaped common headers are reusable

- **WHEN** a header supplies Ed25519, legacy EdDSA or ES256, an optional
  explicit proof type, at most one DID URL `kid`, public JWK or X.509 chain,
  and optional bounded attestation or trust-chain evidence
- **THEN** the codec preserves the validated header without applying DID,
  proof-type, algorithm-selection, certificate, nested-JWT or trust policy
...
- **WHEN** a header duplicates a member, supplies multiple key references, uses
  ...
  private JWK material, uses an invalid X.509/evidence chain, or exceeds a bound
