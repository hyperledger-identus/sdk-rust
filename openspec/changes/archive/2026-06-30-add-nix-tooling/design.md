...
The stub crate and root workspace use `edition = "2024"` with `rust-version = "1.85.0"` (the edition 2024 floor). `resolver = "3"` is set explicitly at the workspace level to match the edition 2024 default, for clarity and for readers who scan the manifest without inferring from edition.
...
Note on the resolver: the `[workspace] resolver` field accepts `"1"`, `"2"`, or `"3"`. Edition 2024 defaults to `"3"` (requires Rust 1.84+), which is resolver 2's feature-unification rules plus changing the default of `resolver.incompatible-rust-versions` from `allow` to `fallback` (the resolver prefers dependency versions whose `rust-version` is ≤ your toolchain). Since the MSRV is `1.85.0` (above the 1.84 floor resolver 3 needs), `resolver = "3"` is set explicitly rather than relying on the edition default, so the manifest records the intended behavior and reviewers don't accidentally downgrade it. Setting `resolver = "2"` would have *downgraded* the workspace off the edition-2024 default; `"3"` is the correct value for an edition-2024 workspace.
