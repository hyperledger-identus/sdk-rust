...
- [ ] 1.1 Implement deterministic runtime/package identity and a canonical
      repository-private cache path outside registered working trees.
- [ ] 1.2 Populate and verify a complete cache without lifecycle-script
      approval, then link Pi's project path without replacing operator data.

## 2. Bootstrap and operations

- [ ] 2.1 Prepare and audit the cache before every bootstrap Pi launch while
      retaining the exact current runtime and package versions.
- [ ] 2.2 Add the raw-Pi ignore fallback and document ownership, concurrency,
      retention, cleanup and recovery.

## 3. Evidence and integration

- [ ] 3.1 Add focused tests for cache identity, external containment,
