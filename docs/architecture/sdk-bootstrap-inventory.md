...
| `identus-oid4vci` | bounded Credential Offer invocation parsing with distinct embedded JSON and HTTPS-reference values, strict form decoding, duplicate-free JSON, caller-configurable limits, static errors and erased owned content | no offer-member semantics, HTTP, redirects, issuer trust, SSRF policy, issuance state machine, authorization, token or credential endpoints | issue #111 under #7 |
