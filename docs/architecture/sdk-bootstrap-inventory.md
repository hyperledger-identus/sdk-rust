...
| `identus-did` | bounded DID/DID URL/URI values, extensible public DID documents, resolution/dereferencing and registration ports/registries, generic dereferencing, typed metadata/errors and injectable resolution caching | featureless runtime cone is `identus-core` plus the `identus-derive` proc macro; structural JWK material only, with no crypto algorithms, method/chain behavior, transport, storage or trust policy | issues #5 and #101 |
...
| `identus-oid4vci` | bounded Final Credential Offer, metadata, pre-authorized Token Request/Response, nonce, configuration-ID/JWT-proof Credential Request, immediate and deferred Credential Response body cores, request-bound immediate response, and Credential payload-error HTTP semantics with static errors and zeroizing secret ownership | no HTTP execution/provenance, deferred request or polling lifecycle, Authorization Details/Credential identifiers, distinct proof-key or credential binding, token/proof/credential/transaction trust or freshness, RFC 6750 Authorization Error parsing, recovery policy, encrypted responses, format/chain extensions, storage or product policy; direct internal cone is `identus-core` plus `identus-jose` | bounded child deliveries ending at issue #149 under #7 |
