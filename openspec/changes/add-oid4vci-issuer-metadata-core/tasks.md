...
- [ ] 2.1 Add metadata limits and static redacted error/core-code variants.
- [ ] 2.2 Extend the selective bounded scanner for required metadata members,
  advertised Authorization Servers, endpoints, and configuration summaries.
- [ ] 2.3 Add unsigned metadata types, safe accessors, exact JSON retention, and
  the consuming `CredentialOfferWithMetadata` agreement transition.

## 3. Evidence and architecture

- [ ] 3.1 Add official Final-shaped, Oxid-shaped, and Lace-shaped positives for
  explicit/default/multiple Authorization Servers and opaque extensions.
- [ ] 3.2 Add malformed/type/duplicate/bound/endpoint/issuer/configuration/hint
  negatives, exact-bound cases, and diagnostic canaries.
- [ ] 3.3 Record the ADR and advance the blueprint/canonical `IDR-023` issue
