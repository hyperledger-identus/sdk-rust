...
| `identus-oid4vci` | bounded Final Credential Offer, metadata, pre-authorized Token Request/Response, typed Token Response credential Authorization Details, nonce, configuration-ID/JWT-proof Credential Request, immediate and deferred Credential Response body cores, request-bound immediate response, and Credential payload-error HTTP semantics with static errors and zeroizing secret ownership | no HTTP execution/provenance, deferred request or polling lifecycle, Credential Request-by-identifier construction, Authorization Request/Code execution, distinct proof-key or credential binding, token/proof/credential/transaction trust or freshness, RFC 6750 Authorization Error parsing, recovery policy, encrypted responses, format/chain extensions, storage or product policy; direct internal cone is `identus-core` plus `identus-jose` | bounded child deliveries ending at issue #237 under #7 |
