...
| `identus-oid4vci` | bounded Final Credential Offer, issuer metadata including nonce and deferred endpoints, pre-authorized Token Request/Response, typed Token Response credential Authorization Details, nonce, configuration-ID and authorized-dataset JWT Credential Requests, immediate and deferred Credential Response body cores, request-bound immediate response, and Credential payload-error HTTP semantics with static errors and zeroizing secret ownership | no HTTP execution/provenance, deferred request or polling lifecycle, Authorization Request/Code execution, request/response correlation, distinct proof-key or credential binding, token/proof/credential/dataset/transaction trust or freshness, RFC 6750 Authorization Error parsing, recovery policy, encrypted responses, format/chain extensions, storage or product policy; direct internal cone is `identus-core` plus `identus-jose` | bounded child deliveries ending at issue #241 under #7 |
