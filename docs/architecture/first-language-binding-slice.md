...
| Kotlin | Go for production slice | Kotlin 2.2.20/JVM 17 host proof, deterministic arm64-v8a AAR proof and a separate test-only x86_64 API-35 emulator receipt using explicit JNA 5.18.1 | ARM64 device execution, physical device, other ABI/runtime matrix, Maven publication and supported JVM/Android API contract |
...
Subsequent issues #228 and #230 now provide local arm64 iOS Simulator and
arm64-v8a Android package evidence. Issue #276 separates the Android behavior
receipt into a test-only x86_64 API-35 emulator job on hosted Linux with KVM.
Still unrun by design: ARM64 Android device execution, physical-device and
wider runtime/ABI tests, React
