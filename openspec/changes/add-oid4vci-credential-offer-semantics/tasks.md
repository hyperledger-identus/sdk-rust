...
- [ ] 2.1 Add semantic limits and static redacted error/core-code variants.
- [ ] 2.2 Implement selective required-member parsing from consumed validated
  transport without materializing unknown values or large numeric extensions.
- [ ] 2.3 Validate the issuer identifier, configuration IDs, and grants shape;
  retain exact JSON and expose only explicit accessors/redacted diagnostics.

## 3. Evidence and architecture

- [ ] 3.1 Add official and independent consumer-shaped positive tests.
- [ ] 3.2 Add missing/type/duplicate/URI/grants/exact-bound/numeric-extension and
  diagnostic-canary negatives plus direct/invocation transition equivalence.
- [ ] 3.3 Update the blueprint and canonical `IDR-023` ledger without claiming
