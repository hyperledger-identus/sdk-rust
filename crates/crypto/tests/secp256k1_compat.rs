//! The 50-vector JVM-compat secp256k1 verification suite, ported from
//! neoprism's `secp256k1_compat.rs`. Signatures come from the JVM PRISM node
//! (bouncycastle/bitcoinj); `verify` SHALL accept them via one of the three
//! fallback paths (raw / normalize-s / bitcoin-transcode).

use std::str::FromStr;

use identus_crypto::Verifiable;
use identus_crypto::hex::HexStr;
use identus_crypto::secp256k1::Secp256k1PublicKey;
...
impl TestInput {
    ...
    fn signature(&self) -> Vec<u8> {
        HexStr::from_str(self.signature)
            .unwrap()
            .to_bytes()
            .to_vec()
    }
...
// ---------------------------------------------------------------------------
// bitcoin-transcode fallback path
// ---------------------------------------------------------------------------
// A signature whose `r` and `s` are serialized in reversed (little-endian)
// byte order — the case the JVM PRISM node's `transcodeSignatureToBitcoin`
// fallback exists to recover. Vanilla and normalize-s verification MUST reject
// it (the scalar byte order is wrong); the bitcoin-transcode fallback MUST
// accept it by reversing each half back to big-endian. This locks in the
// cross-node compatibility contract: before the fix the transcode path parsed
// the reversed raw `r||s` with `Signature::from_der` (which always rejected
// it, since raw bytes carry no DER SEQUENCE tag) and early-returned on
// already-low-s signatures, so the path was unreachable and this vector
// failed.
#[test]
fn secp256k1_bitcoin_transcode_fallback_verifies_reversed_signature() {
    // message = "hello transcode"; key from seed 0x05.
    let message = b"hello transcode";
    let public_key_hex = "022f8bde4d1a07209355b4a7250a5c5128e88b84bddc619ab7cba8d569b240efe4";
    // DER-encoded signature with `r` and `s` in reversed (little-endian) byte
    // order relative to the canonical big-endian encoding.
    let le_signature_hex = "3044022070f099402811ac0ec21c8b0fe1807e990e01899f4b8f6a7e36ab6c0a1a55e18f0220478701d9e8acf4b5e3b9eea8aa80314baf237551270dd9745c8cc172175f9728";

    let pk = Secp256k1PublicKey::from_slice(&HexStr::from_str(public_key_hex).unwrap().to_bytes())
        .unwrap();
    let sig = HexStr::from_str(le_signature_hex)
        .unwrap()
        .to_bytes()
        .to_vec();
    ...
    assert!(
        ...
        "bitcoin-transcode fallback must verify a reversed (LE) r||s signature"
    );
}

// Asserts the first two fallback branches (vanilla and normalize-s) reject
// the reversed signature, so the pass in the test above is genuinely the
// bitcoin-transcode branch deciding — not an accidental accept by an earlier
// path. Uses `k256` directly to exercise the two prior branches in isolation.
#[test]
fn secp256k1_bitcoin_transcode_is_the_deciding_branch() {
    ...
    let pk_bytes = HexStr::from_str(public_key_hex).unwrap().to_bytes();
    let vk: k256::ecdsa::VerifyingKey = k256::PublicKey::from_sec1_bytes(&pk_bytes).unwrap().into();
    let der = HexStr::from_str(le_signature_hex).unwrap().to_bytes();
    let sig = Signature::from_der(&der).unwrap();

    // Branch 1 — vanilla verification: must fail (r is in reversed byte order).
    assert!(
        vk.verify(message, &sig).is_err(),
        "vanilla verify must reject the reversed-r signature"
    );
    // Branch 2 — normalize-s verification: must fail (r is still reversed).
    let normalized = sig.normalize_s().unwrap_or(sig);
    assert!(
        vk.verify(message, &normalized).is_err(),
        "normalize-s verify must reject the reversed-r signature"
    );
}
