...
| `identity_did` | 1.5.1 / signed `v1.5.1` (`917b3815`) | `dependency-cone`, `coupling`, `advisories`, `unsafe-reach`, `target-gap` | Identus DID facade; retain ADR 0104's separately locked manual fixture only. | A narrow published syntax crate has clean denied-advisory evidence, no unsafe/model leakage, and compiles for SDK targets. |
...
| `oauth2` | 5.0.0 / `f3424b4b`; crates.io checksum `51e219e79014df21a225b1860a479e2dcd7cbd9130f4defd4bd0e191ea31d67d` | `dependency-cone`, `coupling`, `semantic-mismatch`, `strictness-mismatch`, `low-payoff` | Keep bounded Identus OID4VCI states and use the exact fixture only as an authorization URL/PKCE/token-request oracle ([ADR 0102](../../adr/0102-use-oauth2-500-as-an-authorization-oracle.md)). | A release feature-slices the needed mechanics from clock/RNG/HTTP/response models, validates PKCE without panic, offers bounded redacted errors, and a named engine proves multi-mechanic payoff. |
| `aries-askar` | 0.4.6 / `e18075d0`; crates.io checksum `17f26462dffcad927db37fc36ef2c49741b222f81aa9f624935c1989d69f72b7` | `dependency-cone`, `coupling`, `native-unsafe`, `publication-readiness`, `low-payoff` | Keep storage ports and revisions SDK-owned; retain the isolated exact-record fixture as reference evidence only ([ADR 0103](../../adr/0103-spike-aries-askar-as-an-isolated-storage-adapter.md)). | A published release feature-slices storage from broad KMS/FFI, supplies atomic revision/CAS semantics, passes clean advisory and native target gates, and a named consumer proves migration/recovery payoff. |
