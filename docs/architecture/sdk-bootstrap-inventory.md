...
| `identus-oid4vci` | bounded Final Credential Offer, metadata, pre-authorized Token Request/Response, nonce, configuration-ID/JWT-proof Credential Request, and request-bound immediate Credential Response semantics with static errors and zeroizing secret ownership | no HTTP execution/provenance, Authorization Details/Credential identifiers, distinct proof-key or credential binding, token/proof/credential trust or freshness, error/deferred/encrypted responses, format/chain extensions, storage or product policy; direct internal cone is `identus-core` plus `identus-jose` | bounded child deliveries ending at issue #143 under #7 |
