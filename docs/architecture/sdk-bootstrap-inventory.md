...
| `identus-did-resolver-http` | fixed state-closed Axum `GET /{did}` router with bounded `Accept` negotiation and W3C status/projection mapping | host-only outer adapter; no resolver method, server runtime, middleware, query options, OpenAPI, portable-target or deployment claim | issue #201 under #10 |
