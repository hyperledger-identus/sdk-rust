...
- [ ] 1.4 Commit the planning-only contract and create the durable preflight
  receipt before implementation.

## 2. Opaque secret boundary

- [ ] 2.1 Make both HD private-key and chain-code fields private without
  changing derivation behavior or public metadata.
- [ ] 2.2 Add the fixed, SDK-owned, redacted and drop-zeroizing exposure value
  plus named private-key/chain-code exposure methods.
- [ ] 2.3 Migrate repository vectors and internal tests through the explicit
  boundary while retaining byte-identical results.
- [ ] 2.4 Add compile-fail field, Display and Serde regressions plus runtime
  redaction and erasure tests.

## 3. Candidate and target evidence

- [ ] 3.1 Update the unpublished `0.1.0-rc.1` public API baseline and record the
