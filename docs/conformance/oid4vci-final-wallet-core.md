...
**SDK revision:** `develop@7f23129a245072e34ea5ddf1e601e14a4b493ed5`
...
metadata, but two required exit conditions are not delivered:

1. the Pre-Authorized Code Token Request has no request-consuming Token
   Endpoint response binder equivalent to the Authorization Code flow; issue
   [#375](https://github.com/hyperledger-identus/sdk-rust/issues/375) owns this
   functional seam;
2. the historical Oxid/Lace ID Portal vector-suite and consumer-review
...
After #375 and #376 land, update the matrix at their exact merged revisions and
re-evaluate #7/M4. No additional feature is justified by this report alone.

## Coverage summary

| Status | Rows | Meaning |
| --- | ---: | --- |
| Implemented | 13 | Bounded public surface, canonical contract and executable tests exist. |
...
| Missing | 2 | A required closeout condition has a focused open owner. |
...
- **Section 6:** both request constructors exist, and the Authorization Code
  flow has request-bound Token Endpoint response plus Authorization Details
  correlation. The equivalent Pre-Authorized Code response transition is the
  functional gap in #375.
...
| --- | --- | --- |
| Wire types and bounded validation | Thirty-seven bounded deliveries, canonical specs and crate tests | Delivered for the recorded wallet-core subset |
